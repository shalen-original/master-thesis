...
% Bibtex parameters to linebreak urls
...
\usepackage[acronym,toc]{glossaries} % TODO: include "acronym" if glossary and acronym should be separated
\makeglossaries{}
...
\lstset{%
  ...
  breaklines=true,
  postbreak=\mbox{\textcolor{TUMLightGray}{$\hookrightarrow$}\space},
}
...
\newcommand{\convertWGStoECEF}[1]{\text{convert}_{\text{EPSG:4979}\rightarrow\text{ECEF}}\left(#1\right)}
\newcommand{\convertECEFtoENU}[2]{\text{convert}_{\text{ECEF}\rightarrow\text{ENU}}\left(#1, #2\right)}
\newcommand{\convertENUtoECEF}[2]{\text{convert}_{\text{ENU}\rightarrow\text{ECEF}}\left(#1, #2\right)}
